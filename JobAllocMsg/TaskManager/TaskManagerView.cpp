...
void CTaskManagerView::OnJobNew() 
{
	...
	if(hwnd != NULL)
	{
		...
		cds.cbData = dwNameLen + 1;
		cds.lpData = (PVOID)cUserName;

		LRESULT lRes = ::SendMessage(hwnd, WM_COPYDATA, (WPARAM)m_hWnd, (LPARAM)&cds);
		...
		if(lRes > 0)
		{
			...
			CString strTmp;
			strTmp.Format(" Nachricricht gesendet : Neue Job N %d\r\n", m_nCurrJob); 
...
void CTaskManagerView::OnJobOk() 
{
	...
	if(hwnd != NULL)
	{
		::PostMessage(hwnd, WM_USER +1, m_nCurrJob, 0);

		CTime t = CTime::GetCurrentTime();
		CString strTime = t.Format("%A %B %d %Y %H %M %S");
		CString strTmp;
		strTmp.Format(" Nachricricht gesendet : Job N %d : OK \r\n", m_nCurrJob); 
		m_strProtocol += strTime + strTmp;
...
void CTaskManagerView::OnJobPause() 
{
	...
	if(hwnd != NULL)
	{
		...
		{
			COPYDATASTRUCT cds;
			cds.dwData = m_nCurrJob;
			
			const char* pc = LPCTSTR(dlg.m_strReason);
			cds.cbData = dlg.m_strReason.GetLength() + 1;
			cds.lpData = (PVOID)pc;
			...
			CString strTime = t.Format("%A %B %d %Y %H %M %S");
			CString strTmp;
			strTmp.Format(" Nachricricht gesendet : Job N  %d ist angehalten.\r\n", m_nCurrJob); 
			m_strProtocol += strTime + strTmp;
//			strTmp.Format("Problem Type: %d\r\n", dlg.m_nProblemType); 
